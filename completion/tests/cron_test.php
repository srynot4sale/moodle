...
class completioncron_testcase extends completion_testcase {
    ...
    public function test_completion_bulk_start() {
        global $CFG, $DB;
        ...
        // Delete all records are recreate using bull functionality
        ...
        $eventdata->course       = $course1;
        $eventdata->startonenrol = !empty($course1->completionstartonenrol);
        completion_handle_start_bulk($eventdata);

        $eventdata->course       = $course2;
        $eventdata->startonenrol = !empty($course2->completionstartonenrol);
        completion_handle_start_bulk($eventdata);

        $this->_check_results();
    }

    private function _check_results() {
        global $CFG, $DB;
...
var_dump($cc2);
var_dump($DB->get_records('user_enrolments', array('userid' => $user2->id)));
print time();
        ...
        $this->assertEquals(0,          $cc2[$user2->id]->timeenrolled);
